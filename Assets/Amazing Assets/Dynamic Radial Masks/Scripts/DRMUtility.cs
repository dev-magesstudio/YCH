...
namespace AmazingAssets.DynamicRadialMasks
{
    ...
    {
        static public string GetMaterialPropertyName(DynamicRadialMasks.Enum.MaskShape shape, int count, DynamicRadialMasks.Enum.MaskType type, DynamicRadialMasks.Enum.MaskBlendMode blendMode, int ID, DynamicRadialMasks.Enum.MaskScope scope, DynamicRadialMasks.Enum.MaskPropertyName propertyName)
        {
        ...
        static public float CalculateValue_Torus(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
                    {
                        float retValue = 1;
                ...
                case DynamicRadialMasks.Enum.MaskBlendMode.Additive:
                    {
                        float retValue = 0;
                        for (int i = 0; i < shapeCount; i++)
                        {
        ...
        static public float CalculateValue_Tube(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static public float CalculateValue_HeightField(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static public float CalculateValue_Dot(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static public float CalculateValue_Shockwave(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static public float CalculateValue_Sonar(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize, float[] maskRingCount, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
                    {
                        ...
                        for (int i = 0; i < shapeCount; i++)
                        {
                            retValue *= 1 - DynamicRadialMasks_Sonar(position, maskPosition[i], maskRadius[i], maskIntensity[i], maskEdgeSize[i], (int)maskRingCount[i], isSimple ? 2 : maskSmooth[i]);
                        }
                ...
                case DynamicRadialMasks.Enum.MaskBlendMode.Additive:
                    {
                        ...
                        for (int i = 0; i < shapeCount; i++)
                        {
                            retValue += DynamicRadialMasks_Sonar(position, maskPosition[i], maskRadius[i], maskIntensity[i], maskEdgeSize[i], (int)maskRingCount[i], isSimple ? 2 : maskSmooth[i]);
                        }
        ...
        static public float CalculateValue_Rings(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskEdgeSize, float[] maskRingCount, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static public float CalculateValue_Ripple(Vector3 position, int shapeCount, DynamicRadialMasks.Enum.MaskType maskType, DynamicRadialMasks.Enum.MaskBlendMode maskBlendMode, Vector4[] maskPosition, float[] maskRadius, float[] maskIntensity, float[] maskPhase, float[] maskFrequency, float[] maskSmooth, bool isSimple)
        {
            switch (maskBlendMode)
            {
                case DynamicRadialMasks.Enum.MaskBlendMode.Normalized:
        ...
        static float DynamicRadialMasks_Sonar(Vector3 vertexWorldPosition, Vector3 maskPosition, float radius, float intensity, float edgeSize, int ringCount, float power)
        {
            if (radius < 0.0001f || intensity < 0.0001f || edgeSize < 0.0001)
                return 0;
