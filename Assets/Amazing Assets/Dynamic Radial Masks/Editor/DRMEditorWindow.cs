    ...
    public class DRMEditorWindow : EditorWindow
    {
        ...
        void DrawMainWindow()
        {
            ...
            if (GUI.Button(new Rect(drawRect.xMax - drawRect.height, drawRect.yMin, drawRect.height, drawRect.height), UnityEditor.EditorGUIUtility.IconContent("_Popup@2x"), DRMEditorResources.GuiStyleIconButton))
            {
                ...
                if(EditorUtilities.GetCurrentRenderPipeline() == EditorUtilities.RenderPipeline.Builtin)
                {
                ...
                menu.AddItem(new GUIContent("Documentation"), false, CallbackMenuDocumentation);
                menu.AddItem(new GUIContent("Forum"), false, CallbackMenuForum);
                menu.AddItem(new GUIContent("Support && Bug Report"), false, CallbackMenuSupportAndBugReport);
                menu.AddItem(new GUIContent("Rate Asset"), false, CallbackMenuRateAsset);
                menu.AddItem(new GUIContent("More Assets"), false, CallbackMenuMoreAssets);
        ...
        string GetTemplateFileLocation(FILE_EXTENTION _Extention, string _MaskShape, string _MaskType, string _MaskBlendMode)
        {
            string fileID = string.Empty;
            switch (_Extention)
            {
                case FILE_EXTENTION.cginc: fileID = _MaskShape; break;
            ...
            string path = Path.Combine(EditorUtilities.GetThisAssetsProjectPath(), "Shaders");
        ...
        void CreateSubGraphFile(DynamicRadialMasks.Enum.MaskShape _MaskShape, int _ShapeCount, DynamicRadialMasks.Enum.MaskType _MaskType, DynamicRadialMasks.Enum.MaskBlendMode _MaskBlendMode, int _MaskID, DynamicRadialMasks.Enum.MaskScope _MaskScope, FILE_EXTENTION _Extention)
        {
            ...
            string saveFolder = Path.Combine(EditorUtilities.GetThisAssetsProjectPath(), "Shaders");
